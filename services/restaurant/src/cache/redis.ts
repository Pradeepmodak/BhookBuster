...
export const connectRedis = async () => {
  ...
  try {
    ...
    client.on("ready", () => {
      ready = true;
      console.log("Restaurant service connected to Redis");
    });
...
export const withCache = async <T>({
  ...
  // Interview note: read-heavy catalog and analytics endpoints use cache-aside
  // so the service stays available even if Redis is temporarily unavailable.
